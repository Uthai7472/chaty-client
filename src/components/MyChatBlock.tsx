...
const MyChatBlock = () => {
    ...
    useEffect(() => {
        const fetchMessages = async () => {
            try {
                const response = await axios.get(`https://chaty-server1.vercel.app/api/chat/show_message`);
                    ...
                    <div key={message.id_table} className={message.username === userLogin ? 'message-container' : 'your-message-container'}>
                        ...
                        <div className='message-block'>
                            ...
                            <div className='message-image-block' onClick={() => openImage(message.image_url)}>
                                {/* {message.image_url} */}
                                <img src={`https://chaty-server1.vercel.app/api/chat/get-image/${message.image_url}`} alt="image" />
                            </div>
                        )}
                        </div>
                    </div>
        ...
        {enlargedImage && (
            <div className='image-overlay' onClick={closeImage}>
                <div className='image-big-size'>
                    <img src={`https://chaty-server1.vercel.app/api/chat/get-image/${enlargedImage}`} />
                </div>
            </div>
        )}
    </div>
